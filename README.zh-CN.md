...
- [`plugins/mface-capture/`](plugins/mface-capture/)：原创 QQ 表情采集替代插件，其 JSONL 元数据不包含聊天文本或发送者 ID，并支持情绪标签审核。可选的本地媒体采集需要运营者同意并设置留存控制。
- 针对 JSONL 元数据遗漏、标签绑定、状态过期、隔离、仅本地接口、容器就绪和 SQLite 行为的测试与运行配置。
