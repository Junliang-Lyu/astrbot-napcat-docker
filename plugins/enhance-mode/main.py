...
from .ban_control import BanStore, offense_to_duration, parse_duration_seconds
...
class Main(star.Star):
    ...
    async def _need_active_reply(
        ...
        if ar.mode == "model_choice":
            ...
            is_rest_time = 12 <= hour < 14 or 18 <= hour < 22
            if not is_rest_time and random.random() > ar.possibility:
                logger.debug(
                    "enhance-mode | active_reply time-gate SKIP | non-rest hour=%d possibility=%.2f",
                    hour, ar.possibility,
                )
                return False
    ...
    async def ban_user(
        ...
        """Ban a user in the current group. Duration is auto-determined by offense history: 1st offense=30m, 2nd=1h, 3rd+=1d. Resets if no offense in 14 days.
        ...
        result = self.ban_store.ban_user(
            scope_id=scope_id,
            user_id=target_user_id,
            source_origin=event.unified_msg_origin,
        )
        final_duration = min(result.duration_seconds, cfg.group_features.ban_max_duration_sec)
        expire_time = datetime.datetime.fromtimestamp(result.expires_at).strftime(
            "%Y-%m-%d %H:%M:%S"
        )
        logger.info(
            "enhance-mode | ban applied | scope=%s user_id=%s offense=%s duration=%s origin=%s",
            scope_id,
            target_user_id,
        ...
        return (
            ...
            f"- Offense count: {result.offense_count} (resets after 14 days without violations)\n"
            f"- Duration: {self._format_duration(final_duration)}\n"
            f"- Expires at: {expire_time}"
        )

    @llm_tool(name="enhance_get_offense_count")
    async def get_offense_count(
        self,
        event: AstrMessageEvent,
        user_id: str,
    ) -> str:
        """Get the offense count for a user. Returns current count and next ban duration if banned again.

        Args:
            user_id(string): Required. Target user ID to check.
        """
        cfg = self._cfg()
        if not cfg.group_features.ban_control_enable:
            return "Ban control is disabled in enhance mode config."
        ...
        scope_id = self._ban_scope_id(event)
        if not scope_id:
            return "This tool is group-scoped. Please call in group chat context."

        target_user_id = str(user_id or "").strip().lstrip("@")
        if not target_user_id:
            return "Invalid `user_id`: empty."

        record = self.ban_store.get_offense_record(scope_id=scope_id, user_id=target_user_id)
        offense_count = record["offense_count"]
        next_duration = offense_to_duration(offense_count + 1)
        if offense_count == 0:
            return (
                f"User `{target_user_id}` has no active offense record in scope `{scope_id}`. "
                f"First ban would be 30m."
            )
        last_ts = record["last_offense_at"]
        last_str = (
            datetime.datetime.fromtimestamp(last_ts).strftime("%Y-%m-%d %H:%M:%S")
            if last_ts else "unknown"
        )
        return (
            f"User `{target_user_id}` offense record in scope `{scope_id}`:\n"
            f"- Offense count: {offense_count}\n"
            f"- Last offense at: {last_str}\n"
            f"- Next ban duration: {self._format_duration(next_duration)}"
        )
    ...
    async def _moderate_message(self, text: str, provider: Provider) -> str:
        """调用 LLM 对消息做意图审核，返回 '安全' | '性骚扰' | '辱骂'。失败时返回 '安全'（放行）。"""
        prompt = (
            "你是内容审核员，判断以下QQ群消息对聊天角色的意图类型。\n\n"
            f"消息：\"{text}\"\n\n"
            "只回答以下之一，不要解释：\n"
            "安全\n"
            "性骚扰\n"
        ...
        try:
            resp = await asyncio.wait_for(
                provider.text_chat(prompt=prompt, session_id=uuid.uuid4().hex, persist=False),
                timeout=6.0,
            )
            ...
            logger.warning("enhance-mode | moderation check failed: %s", e)
            return "安全"

    @filter.platform_adapter_type(filter.PlatformAdapterType.ALL)
    @filter.event_message_type(filter.EventMessageType.ALL, priority=9998)
    async def guard_harassment(self, event: AstrMessageEvent) -> None:
        """消息进入 LLM 前的骚扰/辱骂审核层。"""
        cfg = self._cfg()
        if not cfg.group_features.ban_control_enable:
            return
        ...
        sender_id = str(event.get_sender_id() or "").strip()
        if not sender_id or sender_id in self._get_admin_sid_set():
            return
        ...
        category = await self._moderate_message(text, provider)
        if category == "安全":
            return

        record = self.ban_store.get_offense_record(scope_id=scope_id, user_id=sender_id)
        offense_count = record["offense_count"]

        if category == "辱骂" and offense_count == 0:
            # 首次辱骂：记录警告（1 秒即失效的封禁，仅为 offense_count +1），放行交给阿尔丹冷处理
            self.ban_store.ban_user(
                scope_id=scope_id,
                user_id=sender_id,
                duration_seconds=1,
                source_origin=event.unified_msg_origin,
            ...
            logger.info(
                "enhance-mode | moderation warning recorded | scope=%s user=%s category=%s",
                scope_id, sender_id, category,
            )
            return

        # 性骚扰（无论第几次）或辱骂再犯：直接封禁，消息不传给阿尔丹
        result = self.ban_store.ban_user(
            scope_id=scope_id,
            user_id=sender_id,
            source_origin=event.unified_msg_origin,
        ...
        logger.info(
            "enhance-mode | moderation auto-ban | scope=%s user=%s category=%s offense=%s duration=%s",
            scope_id, sender_id, category, result.offense_count, result.duration_seconds,
        )
        event.stop_event()

    def _get_ats(self, event: AstrMessageEvent) -> list[str]:
        """从消息中提取被 @ 的用户 ID 列表（排除 bot 自身）。"""
        return [
            str(seg.qq)
    ...
    @permission_type(PermissionType.ADMIN)
    @enhance.command("ban", desc="enhance ban @用户  —  手动封禁，按前科自动升档")
    async def admin_ban(self, event: AstrMessageEvent) -> AsyncGenerator[MessageEventResult, None]:
        scope_id = self._ban_scope_id(event)
        if not scope_id:
            yield event.plain_result("仅限群聊使用。")
            return

        target_ids = self._get_ats(event)
        if not target_ids:
            yield event.plain_result("用法：enhance ban @用户")
            return

        cfg = self._cfg()
        lines = []
        for uid in target_ids:
            if not cfg.group_features.ban_allow_admin and uid in self._get_admin_sid_set():
                lines.append(f"{uid}：管理员受保护，跳过。")
                continue
            result = self.ban_store.ban_user(
                scope_id=scope_id,
                user_id=uid,
            ...
            expire_str = datetime.datetime.fromtimestamp(result.expires_at).strftime("%H:%M:%S")
            lines.append(
                f"{uid}：第 {result.offense_count} 次，封禁 {self._format_duration(result.duration_seconds)}（至 {expire_str}）"
            )
            logger.info(
                "enhance-mode | admin ban | scope=%s uid=%s offense=%s duration=%s",
                scope_id, uid, result.offense_count, result.duration_seconds,
            )
        yield event.plain_result("\n".join(lines))

    @permission_type(PermissionType.ADMIN)
    @enhance.command("unban", desc="enhance unban @用户  —  手动解封")
    async def admin_unban(self, event: AstrMessageEvent) -> AsyncGenerator[MessageEventResult, None]:
        scope_id = self._ban_scope_id(event)
        if not scope_id:
            yield event.plain_result("仅限群聊使用。")
            return
        ...
        target_ids = self._get_ats(event)
        if not target_ids:
            yield event.plain_result("用法：enhance unban @用户")
            return

        lines = []
        for uid in target_ids:
            ok = self.ban_store.unban_user(scope_id=scope_id, user_id=uid)
            lines.append(f"{uid}：{'已解封。' if ok else '未在封禁列表中。'}")
            logger.info("enhance-mode | admin unban | scope=%s uid=%s ok=%s", scope_id, uid, ok)
        yield event.plain_result("\n".join(lines))
